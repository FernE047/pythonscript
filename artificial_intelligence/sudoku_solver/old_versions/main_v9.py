...
def is_value_valid(board: BoardData, y: int, x: int, cell_value: CellData) -> bool:
    ...
    for y_offset in range(BLOCK_SIZE):
        for x_offset in range(BLOCK_SIZE):
            if board[0][BLOCK_SIZE * block_y + y_offset][BLOCK_SIZE * block_x + x_offset] == cell_value:
                return False
    if block_y == 0:
        for y_offset in range(BLOCK_SIZE, BOARD_LENGTH):
            if board[0][y_offset][x] == cell_value:
                return False
    ...
    if block_x == 0:
        for x_offset in range(BLOCK_SIZE, BOARD_LENGTH):
            if board[0][y][x_offset] == cell_value:
                return False
...
def solve_sudoku_board(
    ...
    for value in values:
        ...
        if not is_value_valid(board, empty_cell[0], empty_cell[1], cell_value):
            continue
        board[0][empty_cell[0]][empty_cell[1]] = cell_value
