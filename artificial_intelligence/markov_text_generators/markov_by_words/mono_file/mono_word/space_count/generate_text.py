...
def generate_text() -> None:
    ...
    with open(Path(filename) / "c.txt", "r", encoding="UTF-8") as markov_chain_file:
    ...
    for _ in range(WORDS_GENERATED):
        ...
        for index in range(word_quantity):
            file_path = folder_path / f"{index:03d}.txt"
            generated_words.append(generate_word(file_path))
