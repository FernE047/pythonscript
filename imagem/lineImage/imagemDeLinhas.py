...
def coordenadasPregos(quantia,img):
    ...
    for angulo in numpy.arange(0,360,pregosAngulo):
        ...
        pregos.append((int(hip*math.cos(radiano))+hip,int(hip*math.sin(radiano)+hip)))
    return(pregos)
...
while True:
    ...
    for prego in pregos:
        ...
        if(alterou):
            menorPrego=prego
    if(menorValor!=1):
        print('do prego '+str(pregos.index(pregoAtual))+' para o prego '+str(pregos.index(menorPrego)))
        imagemPregos=fazLinha(pregoAtual,menorPrego,imagemPregos,(255,255,255,255))
