...
def main() -> None:
    backInicial = makeBackground(
        "inicial"
    )  # Image.open("C:\\pythonscript\\imagem\\morphManual\\backgroundinicial.png")#
    backFinal = makeBackground(
        "final"
    )  # Image.open("C:\\pythonscript\\imagem\\morphManual\\backgroundfinal.png")#
    frames = [
        "C:\\pythonscript\\imagem\\morphManual\\frames\\" + a
        for a in os.listdir("C:\\pythonscript\\imagem\\morphManual\\frames")
    ]
    frames.pop(0)
    frames.pop()
    frames.pop()
        ...
        for x in range(largura):
            for y in range(altura):
                if frame.getpixel((x, y))[3] == 0:
                    ...
                    frame.putpixel((x, y), interpolate_tuples(inicio, final, total, n))
        print(n)
