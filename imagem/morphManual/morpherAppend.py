...
def makeFrame(n, total, initial, final):
    ...
    print(n)
    frame = Image.open(
        f"C:\\pythonscript\\imagem\\morphManual\\frames\\frame{n + 1:03d}.png"
    )
    for nParte in range(initial, final):
        with open(
            f"C:\\pythonscript\\imagem\\morphManual\\partesConfig\\parte{nParte:02d}Config.txt",
            "r",
            encoding="utf-8",
        ) as file:
            linha = file.readline()
            while linha:
                if linha.find("fundo") != -1:
                    ...
                    novaCor = interpolate_tuples(pixelInicial, pixelFinal, total, n + 1)
...
def main() -> None:
    ultimaCamadaFeita = len(
        os.listdir("C:\\pythonscript\\imagem\\morphManual\\partesConfig")
    )
