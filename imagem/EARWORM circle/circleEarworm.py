...
def cadaMusicaFaca(
    func: Callable[[tuple[str, str]], Image.Image], pastaDir: str
) -> None:
...
def criaPaleta(N):  # cria paleta, ainda não personalizado
    ...
    print(RGB_tuples)
    RGB_tuples = [
        tuple([int(a * 256 - 1) for a in hTr(*color)]) for color in HSV_tuples
    ]
    print(RGB_tuples)
...
def coordenadasPrego(pregoNumber, quantia, tamanho):
    ...
    coord = (
        int(raio * math.cos(radiano)) + raio + 5,
        int(raio * math.sin(radiano) + raio + 5),
    )
    return coord
